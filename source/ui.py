...
"""
Ce fichier contient les fonctions nécessaires à l'interface graphique du
chatbot.
Chaque boite de dialogue est realisée par une fonction.
Il y à trois modes de saisie : soit un mode sur une ligne, soit un mode sur
plusieurs lignes, soit un mode libre.
"""
...
def query_box(mode):
    """
    Affiche la fenêtre permettant d'entrer la requête avec le mode de saisie
    selon le mode donné en argument.
    ...
    if mode == 0:
        ...
        query = enterbox(
            "Enter your free question (without '?'):",
            title,
            default="")
    return query
...
def cancel_box():
    """
    Affiche la fenêtre indiquant que l'utilisateur a appuyé sur le bouton
    d'annulation.
    """
    title = "Chatbot Game of Thrones"
...
def format_error_box():
    """
    Affiche la fenêtre indiquant que l'utilisateur a entré une requête dont le
    format est incorrecte.
    """
    title = "Chatbot Game of Thrones"
    message = ("Your query was void because the format used is"
               "wrong.".center(80) + '\n'
...
def answer_box(mode, query, query_found, answer, answer_list):
    """
    Affiche la fenêtre rappelant la requête entrée par l'utilisateur, la
    réponse donnée par le chatbot ainsi que l'historique des
    questions/reponses.

    :param int mode: le mode selectionné par l'utilisateur.
    :param query: la requête entrée par l'utilisateur.
    :param list(str) query_found: la requête effectuée par le chatbot composée
    du nom de l'individu 1, du nom de la propriété, du nom de l'individu 2.
    :param str answer: la réponse retournée par le chatbot.
    :param list(str) answer_list: l'historique des questions/réponses.
...
def end_box():
    ...
    :return: True si l'utilisateur clique sur "Continue" et False si
    l'utilisateur clique sur "Quit" ou ferme la fenêtre.
    :rtype: bool
    """
...
def not_in_ontology_box():
    """
    Affiche la fenêtre indiquant que l'utilisateur a entré une requête dont les
...
def no_result_box():
    """
    Affiche la fenêtre indiquant qu il n'y à pas de résultat à la requête
    entrée.
    """
    title = "Chatbot Game of Thrones"
